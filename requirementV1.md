...
### 8.2 复习间隔（简化版 + L3 倍增维护）
按最终等级设置 next_review：
- L0：+1 天
- L1：+2 天
- L2：+5 天
- L3：+10 天起，按连续通过次数（`correct_streak`）翻倍：10 → 20 → 40 → 80 天（封顶 80）

L3 维护规则（2026-06-12 起）：
- L3 的词不退役，进入「维护抽查」：每次三项全过，下次间隔翻倍；失败降回 L2、streak 清零，重爬后从 10 天重新开始倍增
- 干净爬升 L0→L3 需 3 次连续通过，故 streak 超出 3 的部分计为 L3 维护通过次数
- 存量 L3 词若已积累较长 streak，下次通过会直接进入对应的长间隔档位

### 8.3 复习与新学任务生成
参数：
- daily_new（默认 5）
- review_cap（默认 8，单次到期复习最多 10 个词；不是每日复习上限）
- relapse_cap（默认 5）

复习任务：
1. **到期复习总数**：所有 next_review ≤ today 的词都应明确展示数量。
2. **单次复习批次**：每次只取最多 min(review_cap, 10) 个，并可由今日页 30 分钟预算进一步降低本次推荐量，避免疲劳。
...
#### 新词学习
每词固定依次完成认识卡（新词认识，无对错）→ Step B（拼写），然后进入下一词。

认识卡（仅新学阶段）：
- 一屏展示单词、音标、全部释义、例句（目标词高亮），进卡自动 TTS 发音，可重听
- 唯一按钮「记住了，去拼写」（支持 Enter）；没有自评，不产生对错
- 不计入回想统计、不进回流、不计连对、不影响等级；真正的检索测试由「新词自动复习」阶段的回想卡承担
...
- **仅学习不升级**：首次新学过程中的认识卡与拼写不影响等级；认识卡无对错，不计入任何统计
...
#### 9.3.1 今日页（任务入口）
- 展示：今日全部待复习数量、30 分钟内推荐复习数量、可新学总数、计划时长
- 入口：开始复习（到期复习 → 回流）
- 如存在未完成学习会话，展示“继续未完成学习”入口，优先恢复云端保存的活跃会话
- 新学入口：跳转到词表页选择新词，不在今日页直接混合开始
- 今日完成后：展示战报摘要（新学/复习/正确率/升级数/最难词）；完成推荐复习批次即可视为今日 30 分钟计划完成，剩余到期词提示明天继续
- 底部展示已学词数 / 总词数
...
- 内容：
  - 认识卡（仅新学阶段）：
    - 单词 + 音标 + 全部释义 + 例句（目标词高亮）+ 自动 TTS 发音
    - 「记住了，去拼写」按钮（Enter），无对错
  - Step A 意思回想卡：
    - 英文 + 自动 TTS 发音
    - "点击显示答案"按钮
    - 自评两按钮（✅ 想对了 / ❌ 没想出来）
...
- 学习设置：
  - 学习强度预设：轻松（3/6/3）、标准（5/8/5）、强化（10/10/10）
  - 每日新学量 daily_new（滑块，3–20）
...
11. 今日页能显示全部到期复习数，且单次复习只取 review_cap 与 30 分钟预算限制内的推荐批次
12. 今日战报与进度页数据正确
