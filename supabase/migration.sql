...
-- 2a. Versioned vocabulary reference sources.
-- A source row may exist before its verified word data is imported; is_complete
-- prevents the UI/documentation from claiming coverage that is not present.
CREATE TABLE IF NOT EXISTS vocabulary_sources (
  source_key TEXT PRIMARY KEY,
  display_name TEXT NOT NULL,
  source_type TEXT NOT NULL CHECK (source_type IN ('textbook', 'curriculum', 'exam', 'corpus')),
  stage_code TEXT NOT NULL CHECK (stage_code IN ('primary', 'junior', 'senior', 'college', 'general')),
...
-- 2b. A word can belong to many independently versioned sources.
-- word_form supports explicitly verified inflections; canonical_word stores the
-- dictionary form. Both values are normalized lowercase strings.
CREATE TABLE IF NOT EXISTS vocabulary_memberships (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  source_key TEXT REFERENCES vocabulary_sources(source_key) ON DELETE CASCADE NOT NULL,
  canonical_word TEXT NOT NULL CHECK (canonical_word = lower(trim(canonical_word))),
  word_form TEXT NOT NULL CHECK (word_form = lower(trim(word_form))),
  coverage_label TEXT NOT NULL DEFAULT '',
  created_at TIMESTAMPTZ DEFAULT now(),
  UNIQUE(source_key, canonical_word, word_form)
);

-- 2c. Corpus frequency is deliberately separate from curriculum/exam membership.
CREATE TABLE IF NOT EXISTS word_frequencies (
  word TEXT NOT NULL CHECK (word = lower(trim(word))),
  source_key TEXT REFERENCES vocabulary_sources(source_key) ON DELETE CASCADE NOT NULL,
  zipf_frequency NUMERIC(5, 3),
  frequency_per_million NUMERIC(14, 6),
  commonness_band TEXT NOT NULL CHECK (commonness_band IN ('high', 'medium', 'low')),
  created_at TIMESTAMPTZ DEFAULT now(),
  PRIMARY KEY(word, source_key)
);
...
ALTER TABLE vocabulary_memberships ENABLE ROW LEVEL SECURITY;
...
DROP POLICY IF EXISTS "Authenticated users can read vocabulary_sources" ON vocabulary_sources;
CREATE POLICY "Authenticated users can read vocabulary_sources"
  ON vocabulary_sources FOR SELECT
  TO authenticated
  USING (true);

DROP POLICY IF EXISTS "Authenticated users can read vocabulary_memberships" ON vocabulary_memberships;
CREATE POLICY "Authenticated users can read vocabulary_memberships"
  ON vocabulary_memberships FOR SELECT
  TO authenticated
  USING (true);

DROP POLICY IF EXISTS "Authenticated users can read word_frequencies" ON word_frequencies;
CREATE POLICY "Authenticated users can read word_frequencies"
  ON word_frequencies FOR SELECT
  TO authenticated
  USING (true);
...
CREATE INDEX IF NOT EXISTS idx_vocabulary_memberships_canonical
  ON vocabulary_memberships(canonical_word);

CREATE INDEX IF NOT EXISTS idx_vocabulary_memberships_form
  ON vocabulary_memberships(word_form);

CREATE INDEX IF NOT EXISTS idx_vocabulary_memberships_source
  ON vocabulary_memberships(source_key);

CREATE INDEX IF NOT EXISTS idx_word_frequencies_band
  ON word_frequencies(commonness_band);
...
-- Reference-source metadata. The Grade 7 textbook index is populated by
-- import_vocabulary_index.sql. The five reviewed curriculum/exam sources are
-- populated by import_official_vocabulary.sql. SUBTLEX-US remains incomplete
-- until a separately licensed frequency import is available.
INSERT INTO vocabulary_sources (
  ...
  (
    'fltrp-grade-7a', '外研版七年级上册', 'textbook', 'junior', 20,
    '仓库内置词表版本', '七年级上册', NULL, NULL,
    '与 supabase/import_grade7a.sql 同源；使用前应确认教材数据授权', false
  ),
  (
    'fltrp-grade-7b', '外研版七年级下册', 'textbook', 'junior', 20,
    '仓库内置词表版本', '七年级下册', NULL, NULL,
    '与 supabase/import_grade7b.sql 同源；使用前应确认教材数据授权', false
  ),
  (
    'moe-primary-2022', '义务教育英语课程标准二级词汇表', 'curriculum', 'primary', 10,
    '2022年版', '小学二级词汇表（505词）', '2022-04-21',
    'https://www.moe.gov.cn/srcsite/A26/s8001/202204/W020220420582349487953.pdf',
    '中华人民共和国教育部', false
  ),
  (
    'moe-junior-2022', '义务教育英语课程标准三级词汇表', 'curriculum', 'junior', 20,
    '2022年版', '初中三级词汇表（累计1600词）', '2022-04-21',
    'https://www.moe.gov.cn/srcsite/A26/s8001/202204/W020220420582349487953.pdf',
    '中华人民共和国教育部', false
  ),
  (
    'moe-high-school-2020', '普通高中英语课程标准词汇表', 'curriculum', 'senior', 30,
    '2017年版2020年修订', '附录2词汇表（3000词）', '2020-06-03',
    'https://www.moe.gov.cn/srcsite/A26/s8001/202006/t20200603_462199.html',
    '中华人民共和国教育部', false
  ),
  (
    'cet-4', '全国大学英语四级考试词表', 'exam', 'college', 40,
    '2016年修订版', 'CET-4', NULL,
    'https://cet.neea.edu.cn/res/Home/1704/55b02330ac17274664f06d9d3db8249d.pdf',
    '全国大学英语四、六级考试委员会', false
  ),
  (
    'cet-6', '全国大学英语六级考试词表', 'exam', 'college', 50,
    '2016年修订版', 'CET-6', NULL,
    'https://cet.neea.edu.cn/res/Home/1704/55b02330ac17274664f06d9d3db8249d.pdf',
    '全国大学英语四、六级考试委员会', false
  ),
  (
    'subtlex-us', 'SUBTLEX-US 英语词频', 'corpus', 'general', 90,
    'Brysbaert & New subtitle corpus', NULL, NULL,
    'https://www.ugent.be/pp/experimentele-psychologie/en/research/documents/subtlexus',
    'Brysbaert & New；导入前需核对数据许可与署名要求', false
  )
